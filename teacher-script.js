...
function saveAttendance() {
    ...
    // Get all student rows - FIXED: Changed from #attendanceTableBody to .student-row
    ...
    // Collect attendance data from each student row
    ...
    const existingIndex = existingAttendance.findIndex(record => 
    ...
    if (existingIndex !== -1) {
        ...
        existingAttendance[existingIndex] = attendanceData;
    } else {
        // Add new record
    ...
    showSuccessPopup(selectedClass, selectedSection, selectedDate, summary);
...
function validateAttendanceData(attendanceData) {
    if (!attendanceData || !attendanceData.students || !Array.isArray(attendanceData.students)) {
        return false;
    }
