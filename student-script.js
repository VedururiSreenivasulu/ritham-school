document.addEventListener('DOMContentLoaded', function() {
        ...
        link.addEventListener('click', function() {
            ...
            if (targetSection) {
                ...
                if (sectionId === 'attendance') {
                    console.log('Attendance section selected, loading attendance data');
    ...
    console.log('Loading attendance data on initialization');
    loadStudentAttendance();
    ...
    if (student) {
        ...
        attendanceData.forEach(record => {
            if (record.class === student.class && 
                ...
                const studentAttendance = record.students.find(s => s.id === studentId);
...
function loadRecentActivities() {
    ...
    const attendanceRecords = attendanceData
        .filter(record => {
            const studentEntry = record.students.find(s => s.id === studentId);
            return studentEntry !== undefined;
        })
        .map(record => {
            const studentEntry = record.students.find(s => s.id === studentId);
            return {
                date: record.date,
                type: 'attendance',
                description: `Attendance marked as ${studentEntry.status}`,
    ...
    const marksRecords = marksData
        ...
        .map(record => ({
            date: record.date,
            type: 'marks',
            description: `${record.subject}: ${record.marks} marks`,
            icon: 'fas fa-star'
        }));

    // Process assignment records
    const assignmentRecords = assignments
        .filter(assignment => 
            assignment.class === localStorage.getItem('studentClass') &&
            assignment.section === localStorage.getItem('studentSection')
        )
        .map(assignment => ({
...
function loadStudentAttendance() {
    ...
    console.log('Attendance counts:', { present: presentCount, absent: absentCount, late: lateCount });
